...
class ProductTemplate(SpedBase, models.Model):
    ...
    #
    #     produto = super(ProductTemplate, self).create(dados)
    ...
    #     res = super(ProductTemplate, self).write(dados)
    ...
    def create_sped_id(self):
        Produto = self.env["sped.produto"]
        for product_id in self.with_context(active_test=False):
            new = Produto.create({
...
class ProductProduct(SpedBase, models.Model):
