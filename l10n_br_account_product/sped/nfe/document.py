...
class NFe200(FiscalDocument):
        ...
        if self.nfref.refNF:
            ...
            nfe_reference.update({
                'inv_related': {
                    ...
                    'fiscal_document_id': fiscal_doc_id[0],
    ...
    def _get_emmiter(self, cr, uid, ids, inv, company, pool, context=None):
        ...
        if emitter_partner_id:
            emmiter.update({
                'emitter_partner_id': emitter_partner_id[0],
            })
        else:
            emmiter.update({
    ...
    def _get_receiver(self, cr, uid, ids, inv, company, nfe_environment, pool, context=None):
        ...
        if receiver_partner_id:
            receiver.update({'receiver_partner_id': receiver_partner_id[0]})
        else:
    ...
    def _get_details(self, cr, uid, ids, i, context=None):
        ...
        pool = self.pool.get('l10n_br_account_product.cfop')
        cfop_ids = pool.search(
            cr, uid, [('code', '=', self.det.prod.CFOP.valor)])

        inv_line['cfop_id'] = cfop_ids[0] if cfop_ids else False

        pool = self.pool.get('product.uom')
        cfop_ids = pool.search(
