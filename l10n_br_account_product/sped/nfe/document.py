...
#                 nfe.infNFe.ide.tpNF.valor = '1'
#
#             # nfe.infNFe.ide.dEmi = datetime.strptime(inv.date_hour_invoice, '%Y-%m-%d %H:%M:%S').date()
#
#             #
#             # Endereço de Entrega ou Retirada
#             #
...
class NFe310(NFe200):
    ...
    def nfe_identification(self, cr, uid, ids, inv, company, nfe_environment, context=None):
        ...
        self.nfe.infNFe.ide.dhEmi.valor = pytz.utc.localize(
            datetime.strptime(inv.date_hour_invoice, '%Y-%m-%d %H:%M:%S')).astimezone(tz) or ''
